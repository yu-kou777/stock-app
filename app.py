...
def get_tickers_safe():
    ...
    try:
        ...
        pass
    if not tickers_dict: tickers_dict.update(NAME_MAP)
...
def detect_candle_pattern(df):
    ...
    if (not is_green1 and body1 > d1.Open*0.01 and body2 < body1*0.3 and 
        ...
        d3.Close > (d1.Open+d1.Close)/2): return "🌅明けの明星", 50, "buy"
    ...
    if (is_green1 and body1 > d1.Open*0.01 and body2 < body1*0.3 and 
        ...
        d3.Close < (d1.Open+d1.Close)/2): return "🌌宵の明星", 50, "sell"
...
def get_analysis(ticker, name, min_p, max_p):
    try:
        ...
        # 補足情報 (板読みの代用)
        # ----------------------------------------
        # MA5より上なら「買い優勢」、下なら「売り優勢」と簡易判定
        status_note = "☁️もみ合い"
        if curr_price > ma5 * 1.01:
            status_note = "📈買い優勢" # 上昇トレンド中
        elif curr_price < ma5 * 0.99:
            status_note = "📉売り優勢" # 下降トレンド中
...
st.title("🦅 最強株スキャナー (補足情報付き)")
st.caption("RSI 60未満 × 強力シグナル × 需給ステータス")
...
with col2:
    st.info("板情報の代わりに「補足」列でトレンドの勢いを表示します。")
    ...
    if data:
        ...
        with c1:
            ...
            if not buys.empty:
                # 列の並び順と名前を指定
                st.dataframe(
                    buys[["code", "name", "signal", "status", "price", "rsi", "buy_target", "res_line"]].rename(
                        columns={
                            "code": "コード",
                            "name": "銘柄名",
                            "signal": "特選シグナル",
        ...
        with c2:
            ...
            if not sells.empty:
                st.dataframe(
                    sells[["code", "name", "signal", "status", "price", "rsi", "sell_target", "sup_line"]].rename(
                        columns={
                            "code": "コード",
                            "name": "銘柄名",
                            "signal": "特選シグナル",
