...
def get_analysis(ticker, name, min_p=0, max_p=10000000):
    try:
        ...
        # RSI計算: $100 - (100 / (1 + \text{Gain} / \text{Loss}))$
        ...
        if not is_risk:
            ...
            if rsi_val < 60:
                ...
                if pattern_name and ("逆三尊" in pattern_name or "三空" in pattern_name):
                    buy_score += pattern_score
            # 売り
            if rsi_val > 65:
                sell_score += 40
                if "下降" in trend_label: sell_score += 20
                if pattern_name and "三尊" in pattern_name:
...
if target_code:
    ...
    if res:
        ...
        if res["is_risk"]:
            st.error("⚠️ 決算が近いため、テクニカルが良くてもエントリーは控えるのが安全です。")
    else:
